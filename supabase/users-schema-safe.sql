-- Safe Users Schema - Only creates what doesn't exist
...
drop policy if exists "super_admins can update users" on public.users;
drop policy if exists "super_admins can delete users" on public.users;
...
-- Create or replace triggers (safe to run multiple times)
